...
function findClosestNumber(nums: number[]): number {
    ...
    let n_middle: number = Math.ceil((nums.length - 1) / 2);
    for (let i = 1; i <= n_middle; i++) {
        const left_side: number = nums[i];
        if (Math.abs(left_side) < Math.abs(closest) || (Math.abs(left_side) === Math.abs(closest) && left_side > closest)) 
            closest = left_side


        const right_side: number | undefined = nums[n_middle + i];
        if (right_side !== undefined) {
            if (Math.abs(right_side) < Math.abs(closest) || (Math.abs(right_side) === Math.abs(closest) && right_side > closest))
                closest = right_side;
        }
    }
